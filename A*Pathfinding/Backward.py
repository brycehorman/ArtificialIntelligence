...
def randGridMaze(number, width=101, height=101):
    ...
    Z, forwardNum = repeatedAStarForward(Z, start, end)
    backwardNum = repeatedAStarBackward(Z, start, end)[1]
...
def repeatedAStarForward(z, start, end):
    finalpath = [start]
    ...
    print('Forward Number of expanded nodes:', numExpanded)
    

    return [z, numExpanded]

def repeatedAStarBackward(z, start, end):
    finalpath = [start]
    S = np.empty(z.shape, dtype=object)
    for i in range(z.shape[1]):
        for j in range(z.shape[0]):
    ...
    while(start != end):
        ...
        o = []
        c = []
        heapq.heappush(o, (S[end].f, S[end].g, 0, S[end]))
        if(end[1] + 1 < z.shape[1]):
            z2[end[0],end[1]+1] = z[end[0],end[1]+1]
        if(end[1] -1 >= 0):
            z2[end[0],end[1]-1] = z[end[0],end[1]-1]
        if(end[0]+1 < z.shape[0]):
            z2[end[0]+1,end[1]] = z[end[0]+1,end[1]]
        if(end[0]-1 >=0):
            z2[end[0]-1,end[1]] = z[end[0]-1,end[1]]
        o, c, S, numExpanded = computePathLargestG(o, c, S, z2, end, start, counter, numExpanded)
        if(len(o) == 0):
            print('Target cannot be reached')
            return [z, numExpanded+1]
        initialpath = [start]
    ...
    z[end] = -2
    print('target reached')
    print('Backward Number of expanded nodes:', numExpanded)
...
if __name__ == "__main__":
    ...
    for x in nn:
        totalBackward+=x[1]
        totalForward+=x[0]
    print("Average of forward G:", totalForward/len(nn))
    print("Average of backward G:", totalBackward/len(nn))
