...
def randGridMaze(number, width=101, height=101):
    ...
    Z, largestNum = repeatedAStar(Z, start, end, 'Largest')
    smallestNum = repeatedAStar(Z, start, end, 'Smallest')[1]
...
def repeatedAStar(z, start, end, gType):
    finalpath = [start]
    ...
    while(start != end):
        ...
        if(gType == 'Largest'):
            ...
            o, c, S, numExpanded = computePathSmallestG(o, c, S, z2, start, end, counter, numExpanded)
        if(len(o) == 0):
    ...
    print(gType, 'G Number of expanded nodes:', numExpanded)
...
def computePathSmallestG(o, c, S, z, start, end, counter, numExpanded):
    identifier = 1
        ...
        for a in actions:
            ...
            if(a[0] >= 0 and a[0] < S.shape[0] and a[1] >= 0 and a[1] < S.shape[1] and z[a] != 1):
                ...
                if(S[a].g > smallestF.g + 1):
                    ...
                    if(isInOpen(S[a], o)):
                        continue
                    heapq.heappush(o, (S[a].f, S[a].g, identifier, S[a]))
...
if __name__ == "__main__":
    ...
    for x in nn:
        totalSmall+=x[1]
        totalLarge+=x[0]
    print("Average of smallest G:", totalSmall/len(nn))
    print("Average of largest G:", totalLarge/len(nn))
